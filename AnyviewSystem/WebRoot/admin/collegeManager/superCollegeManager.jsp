...
$(function(){
	$("#collegeManagerCeSelect").select2({
		placeholder : "请选择学校",
		allowClear : true 
	});
}); 
	...
	<div class="searchBar">
		<table class="searchContent" style="float: left;">
			<tr>
				<td>
					学校:
					<select id="collegeManagerCeSelect" name="condition.university.unID" style="width: 150px;">
						 <option></option>
						<c:forEach items="${university }" var="u">
							<option value="${u.unID}" <c:if test="${condition.university.unID==u.unID }">selected="selected"</c:if> >${u.unName }</option>
						</c:forEach>
					</select>
				</td>
				<td>
					学院:
					<input class="college textInput" type="text" name="condition.ceName" maxlength="20" alt="输入学院名称(可选)" value="${condition.ceName }"/>
				</td>
				<td>
					有效状态:
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li >
				<a class="add" mask="true" target="dialog"   fresh="true" rel="admin/collegeManager" href="admin/collegeManager/preAddCollege.action?">
					<span>添加学院</span>
				</a>
			</li>
