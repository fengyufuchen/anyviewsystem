...
$(function(){
	//初始化学校，学院select
	$("#superAdminManagerUnSelect").select2({
		placeholder : "请选择学校",
		allowClear : true
	});
	$("#superAdminManagerCeSelect").select2({
		placeholder : "请选择学院",
		allowClear : true
	});
	//级联
	$("#superAdminManagerUnSelect").on("change",function(e){
		if(this.value==""){
			$("#superAdminManagerCeSelect").clear();
			return;
		}
		$.ajax({
			type:"POST",
			url:"communion/gainCollegeByUnIdAjax.action",
			data:{unId:$("#superAdminManagerUnSelect").val()},
			cache:"false",
			success:function(data)
			{
				//先清空原option
				$("#superAdminManagerCeSelect").html(null);
				var colleges = $.parseJSON(data);
				$("#superAdminManagerCeSelect").append($('<option></option>'));
				for(var i=0; i<colleges.length; i++){
					var opt = $('<option></option>');
					opt.attr("value",colleges[i].id);
					opt.html(colleges[i].text);
					$("#superAdminManagerCeSelect").append(opt);
				}
			}
		});
	});
	...
	<div class="searchBar">
		<table class="searchContent" style="float: left;">
			<tr>
				<td>
					学校:
					<select id="superAdminManagerUnSelect" name="condition.university.unID" style="width: 150px;">
						<option></option>
						<c:forEach items="${universities }" var="u">
							<option value="${u.unID }" <c:if test="${(!empty condition.university) && condition.university.unID==u.unID }">selected="selected"</c:if> >${u.unName }</option>
						</c:forEach>
					</select>
				</td>
				<td>
					学院:
					<select id="superAdminManagerCeSelect" name="condition.college.ceID" style="width: 150px;">
						<option></option>
		...
		<tbody>
			<c:forEach items="${page.content}" var="admin">
				<tr target="tar_mid" rel="${admin.mid}">
					...
					<td align="center">
						<c:choose>
							<c:when test="${admin.college.ceID==-1 }">--</c:when>
							<c:otherwise>${admin.college.ceName }</c:otherwise>
						</c:choose>
					</td>
					<td align="center">
					    <c:if test="${admin.miden==0 }">院级管理员</c:if>
					    <c:if test="${admin.miden==1 }">校级管理员</c:if>
