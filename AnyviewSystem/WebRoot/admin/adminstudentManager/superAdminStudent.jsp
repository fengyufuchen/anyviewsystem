...
$(function(){
	//初始化学校，学院select
	$("#superAdminManagerUnSelected").select2({
		placeholder : "请选择学校",
		allowClear : true
	});
	...
	//级联
	$("#superAdminManagerUnSelected").on("change",function(e){
		if(this.value==""){
			$("#superAdminManagerCeSelected").html(null);
			$("#superAdminManagerCeSelected").val(null);
			$("#superAdminManagerCeSelected").select2({
				placeholder : "请选择学院",
				allowClear : true
			});
		...
		$.ajax({
			...
			data:{unId:$("#superAdminManagerUnSelected").val()},
			cache:"false",
			success:function(data)
			{
				...
				$("#superAdminManagerCeSelected").append($('<option></option>'));
				for(var i=0; i<colleges.length; i++){
					var opt = $('<option></option>');
					opt.attr("value",colleges[i].id);
					opt.html(colleges[i].text);
					$("#superAdminManagerCeSelected").append(opt);
				}
			}
		});
	});
...
<form id="searchClass" rel="pagerForm" onsubmit="return navTabSearch(this);" action="admin/adminstudentManager/getAdminStudentManagerPage.action" method="post">
     ...
     <input type="hidden" name="orderDirection" value="${orderDirection}" />
			...
			<tr>
				<td>&nbsp;&nbsp;&nbsp;&nbsp;学校：
					<select id="superAdminManagerUnSelected" name="condition.college.university.unID" style="width: 150px;">
						<option></option>
	...
	<table class="table" width="100%" layoutH="138">
		<thead>
			<tr>
				...
				<th width="15%" align="center">超管学校</th>
